...
Definition STs_traces (P : Prog_wf) (t : STs) (tr : traces) : Prop := forall k t', 
  MapsTo k t' t -> exists tr', MapsTo k tr' tr /\ ST_trace P (remove k tr) t' tr'.

Program Definition has_ST (c : val) (t : ST) : tasn := 
  mk_tasn (fun trs => mk_asn (fun P k h => forall P' tr, 
    Prog_wf_sub P P' -> 
    MapsTo (val_to_stptr c) tr trs -> 
    ST_trace P' (remove (val_to_stptr c) trs) t tr
  ) _ _ _) _.
Next Obligation.
