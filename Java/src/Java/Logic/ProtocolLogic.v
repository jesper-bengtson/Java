...
Require Import Lang SessionType Util Program.
...
Print protocol.
Locate ST.
  Definition pasn := ILPreFrm (@rel protocol (@Equal channel _ _ ST)) asn.
  
  Instance ILogicOpsPAsn    : ILogicOps pasn    := _.
  Instance ILogicPAsn       : ILogic pasn       := _.  
  Instance BILOperatorsPAsn : BILOperators pasn := @SABIOps2 protocol Equal _ _ asn _ _ _.
  
  Instance BILogicPAsn      : BILogic pasn := @SABILogic2 protocol Equal _ _ asn _ _ _ _ _.

  Instance EmbedPAsnPropOp  : EmbedOp Prop pasn := @EmbedILPreDropOp protocol Equal Prop asn _ _ _.
  Instance EmbedPAsnProp    : Embed Prop pasn   := @EmbedILPreDrop protocol Equal _ Prop _ asn _ _ _ _.
    
  Instance EmbedPAsnAsnOp   : EmbedOp asn pasn  := @EmbedILPreDropOp protocol Equal asn asn _ _ _.
  Instance EmbedPAsnAsn     : Embed asn pasn    := @EmbedILPreDrop protocol Equal _ asn _ asn _ _ _ _.
...
Definition mk_pasn (f: protocol -> heap -> Prop)
  (HProt: forall Pr Pr' h, Pr === Pr' -> f Pr h -> f Pr' h)
  (Hheap: forall Pr h h', subheap h h' -> f Pr h -> f Pr h') : pasn.
  refine (mkILPreFrm (fun Pr => mk_asn (f Pr) (Hheap Pr)) _).
Proof.
  intros Pr Pr' HPr.
...
Defined.
  ...
  Program Definition has_ST (v : var) (T : ST) : psasn :=
    fun s => mk_pasn (fun p h =>
      MapsTo (val_to_channel (s v)) T p
    ) _ _.
  Next Obligation.
	intros.
	rewrite <- H.
  ...
  Program Definition has_subst_ST (x z y : var) (T : ST) : psasn :=
    fun s => mk_pasn (fun p h =>
      MapsTo (val_to_channel (s x)) (subst_ST z (s y) T) p
    ) _ _.
  Next Obligation.
