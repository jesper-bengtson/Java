...
Definition isField (Pr : Program) (ptr : ptr) (f : field) : Prop := forall (cls : class) (pos : nat), 
	exists fields, ptr = (pos, cls) /\ field_lookup Pr cls fields /\ List.In f fields.


Inductive pMarshal (Pr : Program) (ref : ptr) : list field -> heap -> heap -> Prop :=
  | pMarshal_empty : forall (big : heap) fl,
                     fl = nil ->
                     pMarshal Pr ref fl big (empty _)
  | pMarshal_scalar   : forall (big m m' : heap) (f : field) (fl fl' : list field) (v : val),
                     ~ List.In f fl ->
  					 isField Pr ref f ->
  					 MapsTo (ref, f) v big ->
  					 val_to_ptr v = pnull ->
  					 ...
  					 fl' = (f :: fl) ->
                     pMarshal Pr ref fl' big m'
  | pMarshall_ptr :  forall (big m m' m'' m_c : heap) (f : field) (fl fl' fl_c : list field) (cls : class) (pos : nat),
                     List.In f fl ->
                     isField Pr ref f ->
...
Inductive Marshal (Pr : Program) (v : val) (big : heap) : heap -> Prop :=
